...
function [Sig_s,mu_s,v_s,class_id,Sigmas]=unconstrained_tuning(X,Y,att_seen,att_unseen,unseenclasses,K,Psi,mu0,m,k0,k1)
...
for i=1:ncl
    ...
    if nci>0
        ...
        % The case where there is only one class
        else
            v_s(cnt)        = cur_n+m-d+1;
